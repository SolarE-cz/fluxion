...
use fluxion_core::{FluxionCorePlugin, SystemConfig, WebQuerySender};
...
fn initialize_and_run() -> Result<()> {
    ...
    for inv in &config.inverters {
        info!(
            "     - {} ({}) - {} [{}]",
            inv.id, inv.vendor, inv.entity_prefix, inv.topology
        );
    }
    ...
    // For MVP, we only support Solax
    let mapper: Arc<dyn fluxion_core::VendorEntityMapper> = Arc::new(SolaxEntityMapper::new());
    ...
    let language = Language::from_code(&config.system.language).unwrap_or_else(|e| {
        warn!(
            "Invalid language code '{}': {}. Defaulting to English.",
            config.system.language, e
        );
        Language::English
    });
    let i18n = Arc::new(I18n::new(language).expect("Failed to initialize i18n"));
    info!("🌍 Initialized i18n with language: {:?}", language);

    // Spawn web server on tokio runtime
    info!("🌐 Starting web server on port 8099...");
    let i18n_for_server = i18n.clone();
    tokio::spawn(async move {
        if let Err(e) = fluxion_web::start_web_server(query_sender, i18n_for_server, 8099).await {
