...
use crate::{components::*, debug::DebugModeConfig, resources::SystemConfig};
...
fn build_dashboard_response(
    ...
    let price_data_result = price_data
        ...
        .map(|(prices, analysis)| {
            ...
            // Log mismatch if any
            if let Some(s) = &sched {
                let price_blocks = prices.time_block_prices.len();
                let schedule_blocks = s.scheduled_blocks.len();
                if price_blocks != schedule_blocks {
                    warn!(
                        "⚠️ Block count mismatch! Price data: {} blocks ({:.1}h), Schedule: {} blocks ({:.1}h)",
                        price_blocks,
                        price_blocks as f32 / 4.0,
                        schedule_blocks,
                        schedule_blocks as f32 / 4.0
                    );
                }
            }
                    ...
                    let (block_type, target_soc, strategy, profit, reason, debug_info) = sched
                        ...
                        .map(|sb| {
                            ...
                            let target_soc = match sb.mode {
                                InverterOperationMode::ForceCharge => Some(system_config.control_config.max_battery_soc),
                                InverterOperationMode::ForceDischarge => Some(system_config.control_config.min_battery_soc),
                                InverterOperationMode::SelfUse => None,
                            };
                            (block_type_str.to_string(), target_soc, strat, prof, Some(sb.reason.clone()), sb.debug_info.clone())
                        })
                        .unwrap_or_else(|| {
                            ...
                            let (block_type, target_soc) = if analysis.charge_blocks.contains(&idx) {
                                ("charge".to_string(), Some(system_config.control_config.max_battery_soc))
                            } else if analysis.discharge_blocks.contains(&idx) {
                                ("discharge".to_string(), Some(system_config.control_config.min_battery_soc))
                            } else {
