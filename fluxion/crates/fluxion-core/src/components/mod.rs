...
impl InverterModel {
    /// Parse inverter model from vendor string
    /// Returns default model if vendor is unknown or not specified
    pub fn from_vendor(vendor: &str) -> Self {
        match vendor.to_lowercase().as_str() {
            "solax" => Self::SolaxX3HybridG4,
            // Add more vendors as they're supported:
            // "fronius" => Self::FroniusSymo,
            // "sma" => Self::SmaSunnyBoy,
            _ => {
                tracing::warn!("Unknown vendor '{vendor}', defaulting to SolaxX3HybridG4");
                Self::SolaxX3HybridG4
            }
        }
    }
}
